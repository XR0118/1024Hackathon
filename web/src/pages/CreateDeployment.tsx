...
import { versionApi, applicationApi, environmentApi, deploymentApi } from "@/services/api";
...
const CreateDeployment: React.FC = () => {
  ...
  const loadData = useCallback(async () => {
    try {
      const [versionsData, appsData, envsData] = await Promise.all([versionApi.list(), applicationApi.list(), environmentApi.list()]);
          ...
          {grayscaleEnabled && (
            <div className="mb-3">
              ...
              <input
                ...
                onChange={(e) => setGrayscaleRatio(parseInt(e.target.value, 10))}
              />
            </div>
          )}
          <div className="mb-3">
        ...
        <div>
          ...
          <div className="card">
            <div className="card-body">
              <p>
                <strong>版本:</strong> {versions.find((v) => v.version === selectedVersion)?.version}
              </p>
              <p>
                <strong>应用:</strong> {selectedApps.map((name) => applications.find((a) => a.name === name)?.name).join(", ")}
              </p>
              <p>
                <strong>环境:</strong> {selectedEnvs.map((id) => environments.find((e) => e.id === id)?.name).join(", ")}
              </p>
              <p>
                <strong>需要人工确认:</strong> {requireConfirm ? "是" : "否"}
              </p>
              <p>
                <strong>灰度发布:</strong> {grayscaleEnabled ? `是 (${grayscaleRatio}%)` : "否"}
              </p>
              <p>
                <strong>自动回滚:</strong> {autoRollback ? "是" : "否"}
              </p>
            </div>
          </div>
        </div>
  ...
  return (
    <div>
      ...
      <div className="card">
        ...
        <div className="card-body">{steps[currentStep].content}</div>
